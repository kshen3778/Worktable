...
class DenseLayer(nn.Sequential):
    def __init__(self, in_channels, growth_rate):
        ...
        self.add_module('relu', nn.PReLU(in_channels))
...
def center_crop(layer, max_depth, max_height, max_width):
    _, _, z, h, w = layer.size()
    xy1 = (w - max_width) // 2
    xy2 = (h - max_height) // 2
    xy3 = (z - max_depth) // 2

    return layer[:, :, xy3:(xy3 + max_depth), xy2:(xy2 + max_height), xy1:(xy1 + max_width)]
