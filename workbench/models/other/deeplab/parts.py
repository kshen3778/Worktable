...
class Classifier_Module(nn.Module):
    def __init__(self, dilation_series, padding_series, num_classes, factor=4):
        ...
        for dilation, padding in zip(dilation_series, padding_series):
            ...
            self.bn3d_list.append(nn.BatchNorm3d(256 // factor, affine=affine_par))
        self.num_concats = len(self.conv3d_list) + 2
        # add global pooling, add batchnorm
        self.conv1x1_1 = nn.Conv3d(
            2048 // factor, 256 // factor, kernel_size=1, stride=1
        )
        self.conv1x1_2 = nn.Conv3d(
            2048 // factor, 256 // factor, kernel_size=1, stride=1
        )
        self.conv1x1_3 = nn.Conv3d(
            256 // factor * self.num_concats, 256 // factor, kernel_size=1, stride=1
        )
        self.conv1x1_4 = nn.Conv3d(256 // factor, num_classes, kernel_size=1, stride=1)

        self.bn1 = nn.BatchNorm3d(256 // factor, affine=affine_par)
        self.bn2 = nn.BatchNorm3d(256 // factor * self.num_concats, affine=affine_par)
        self.bn3 = nn.BatchNorm3d(256 // factor, affine=affine_par)
        # global avg pool
    ...
    def forward(self, x):
        ...
        self.glob_avg_pool = nn.AvgPool3d(
            kernel_size=(x.size()[2], x.size()[3], x.size()[4])
        )
        self.iterp_orig = nn.Upsample(
            size=(out.size()[2], out.size()[3], out.size()[4]), mode="trilinear"
        )

        out = torch.cat(
            [out, self.iterp_orig(self.bn1(self.conv1x1_1(self.glob_avg_pool(x))))], 1
        )
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes, factor=1):
        self.inplanes = 64 // factor
        super(ResNet, self).__init__()
        self.conv1 = nn.Conv3d(
            1, 64 // factor, kernel_size=3, padding=1, bias=False
        )  # / 2
        self.bn1 = nn.BatchNorm3d(64 // factor, affine=affine_par)
        for i in self.bn1.parameters():
            i.requires_grad = False
        self.relu = nn.ReLU(inplace=True)
        ...
        self.layer2 = self._make_layer(block, 128 // factor, layers[1], stride=1)  # / 8
        self.layer3 = self._make_layer(
            block, 256 // factor, layers[2], stride=1
        )  # / 16

        self.layer4_0 = self._make_layer(
            block, 512 // factor, layers[3], stride=1, dilation__=2
        )
        self.layer4_1 = self._make_layer(
            block, 512 // factor, layers[3], stride=1, dilation__=4
        )
        self.layer4_2 = self._make_layer(
            block, 512 // factor, layers[3], stride=1, dilation__=8
        )
        self.layer4_3 = self._make_layer(
            block, 512 // factor, layers[3], stride=1, dilation__=16
        )

        self.layer5 = self._make_pred_layer(
            Classifier_Module, [3, 6, 9, 12], [3, 6, 9, 12], num_classes,
        )
